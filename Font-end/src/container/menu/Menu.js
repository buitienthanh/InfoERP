...
export default function Menu(props) {
    ...
    const menuList = arr.map((v, i) => {
        return (
            <MenuLink key={i} name={t(v.name)} executeScroll={executeScroll} refer={ref[i]} i={i} />
        )
    })

    return (
        <div>
            <Navbar fixed='top' className="ms-3" collapseOnSelect expand="lg" variant="dark" id='menuContainer'>
            <Container>
